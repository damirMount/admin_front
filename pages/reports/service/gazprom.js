...
export default function DealerExportPage() {
    ...
    const tableColumns = [
        ...
        },
        {
            title: 'Район',
            dataIndex: 'district',
            render: (text, record) => record && record.label === 'ИТОГО:' ? '' : text
        },
        {
    ...
    const handleCreateReport = async () => {
        try {
            ...
            const response = await fetch(GET_GAZPROM_REPORT_API, {
                method: 'POST',
                headers: {
    ...
    const handleDownload = async () => {
        ...
        try {
            const response = await fetch(`${DOWNLOAD_GAZPROM_REPORT_API}`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
            ...
            if (response.ok) {
                ...
                a.href = blobUrl;
                a.download = `gazprom-online-registry-${oldFormData.clientType}-payments-${oldFormData.startDate}-${oldFormData.endDate}`;
    ...
    return (
        <ProtectedElement allowedPermissions={'reports_management'}>
            <div>
                <Head>
                    <title>Итоговый отчет по Газпром онлайн | {process.env.NEXT_PUBLIC_APP_NAME}</title>
                </Head>
                <div className="w-100 mt-5">
                    <h1>Итоговый отчет по Газпром онлайн</h1>
