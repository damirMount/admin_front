...
const RegistryForm = ({oldFormData = {fields: []}, formData = {fields: []}, onDataFieldsChange}) => {
    ...
    const tourSteps = [{
        ...
        title: 'Тип отправки', description: <p>Существуют два метода отправки данных: по услугам и по серверу.:
            <br/>
            ...
            генерироваться в одном файле, без разделения по услугам.</p>, target: () => refSendType.current,
    }, {
        title: 'Формат файлов',
        description: 'Здесь вы выбираете в каком формате вы хотите чтобы генерировался реестр. Вы можете выбрать ' + 'как один формат так и всё сразу. Учтите для каждого формата вам необходимо будет заполнять свои поля ' + 'в таблице ниже',
        target: () => refFormat.current,
    }, {
        title: 'Сервер и услуги',
        description: 'Здесь вы можете выбрать сервер и услуги по которым по которым будут отправляться реестры. ' + 'В случаи если в "Типе отправки" вы указали отправку по серверу вам нужно будет выбрать только сервер. ' + 'В другом случаи вы также сможете выбрать по каким конкретно услугам вы хотите получать данные',
        target: () => refServerAndServices.current,
    }, {
        title: 'Настройка отображение данных в реестре',
        description: 'Эта таблица позволяет настроить, как будут отображаться данные в реестре. Для этого необходимо ' + 'добавить заголовки, по которым данные будут распределяться. Каждый заголовок требует индивидуальной ' + 'настройки, которая может изменяться в зависимости от выбранного формата реестра.',
        target: () => refTable.current,
    }, {
        title: 'Поле в базе',
        description: 'Укажите здесь поле из базы, из которой вы хотите получить данные. ' + 'Данное поле не обязательно заполнять в случаях когда вы указали значение в поле "Стандартное значение". ' + 'Во всех остальных случаях это поле обязательно к заполнению',
        target: () => refField.current,
    }, {
        title: 'Название поля',
        description: 'Здесь вы указываете названия столбца в реестре. Данное поле заполняется только если вы ' + 'используете формат файла XLSX или CSV. Это поле обязательно для заполнения',
        target: () => refFieldName.current,
    }, {
        title: 'Название поля для DBF',
        description: 'В данном поле вы также указываете название столбца в реестре, только уже для DBF файла. ' + 'Учтите что количество символов в DBF файле ограничено. Вы можете ввести либо 10 символов на английском, ' + 'либо 5 символом на русском. Это поле обязательно для заполнения',
        ...
        description: 'Здесь вы можете установить значение по умолчанию для данного поля. ' + 'Существует два вида значений по умолчанию: простое значение и список значений. ' + 'Когда вы устанавливаете простое значение, оно будет применяться ко всем пустым полям. В случае списка ' + 'вы можете указать конкретные значения, которые будут заменяться на другие. Остальные значения, ' + 'не указанные в списке, останутся неизменными.',
        target: () => refRegularValue.current,
    }, {
        title: 'Новое поле',
        placement: 'top',
        description: 'Также у вас имеется возможность добавлять новые поля в реестр! Вы можете выбрать один из ' + 'доступных шаблонов из списка либо же добавить полностью пустое поле.',
    ...
    const handleTableInputChange = (value, record, name) => {
        ...
        // Если в стейте пусто, берем дефолтные поля как стартовую точку
        const currentFields = newData.fields && newData.fields.length > 0
            ? newData.fields
            : cloneDeep(defaultFields);

        newData.fields = currentFields.map(item => {
    ...
    const handleDelete = (key) => {
        ...
        newData.fields = currentFields.filter((item) => item.key !== key);
        onDataFieldsChange(newData);
    };
    ...
    const handleRowSelect = (record) => {
        if (!activeInput && selectedRowKey !== record.key) {
    ...
    const addRow = () => {
        ...
        // Генерируем уникальный ключ на основе текущего времени и рандома
        const uniqueKey = `new_row_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;

        if (matchingItems.length > 0) {
            // Клонируем объект из шаблона, чтобы не сломать исходный defaultFields, и добавляем key
            const fieldToAdd = cloneDeep(matchingItems[0]);
            fieldToAdd.key = uniqueKey;
            newData.fields.push(fieldToAdd);
        } else {
            newData.fields.push({
                key: uniqueKey, // ОБЯЗАТЕЛЬНО ДОБАВЛЯЕМ КЛЮЧ СЮДА
                field: 'account.',
                name: '',
                nameDbf: '',
                formatDbf: 'C',
    ...
    const tableColumns = [{
        ...
        render: (text, record) => {
            if (record.key === selectedRowKey) {
                return <Input
                    value={text || ''} // ИСПРАВЛЕНО: контролируемый компонент
            ...
            return (<span
                className={oldFormData?.fields?.some(filedValue => filedValue.key === record.key && filedValue.field === text) ? '' : 'fst-italic text-decoration-underline'}>
                        {text ? <b>{text}</b> : <span className="text-secondary">(Пусто)</span>}
                    </span>)
        },
    }, {
        ...
        render: (text, record) => {
            if (record.key === selectedRowKey) {
                return <Input
                    value={text || ''} // ИСПРАВЛЕНО: контролируемый компонент
                    placeholder={'(Пусто)'}
                    disabled={formData && formData.fields && !formData.formats?.some(format => ['xlsx', 'csv'].includes(format))}
            ...
            return (<span
                className={oldFormData?.fields?.some(filedValue => filedValue.key === record.key && filedValue.name === text) ? '' : 'fst-italic text-decoration-underline'}>
        ...
        render: (text, record) => {
                ...
                return (<div className='d-flex w-100'>
                    ...
                    <Tooltip title='Формат DBF'>
                        <Select
                            className='ms-2 w-50'
                            placeholder='пусто'
                            value={record.formatDbf || 'C'} // ИСПРАВЛЕНО: контролируемый компонент
                            disabled={formData && formData.fields && !formData.formats?.includes('dbf')}
                            onChange={(value) => {
                                setActiveInput(null);
                            ...
                            onFocus={() => setActiveInput(record.key)}
                            onBlur={() => setActiveInput(null)}
                            options={[{value: 'C', label: 'CHAR'}, {value: 'N', label: 'NUMERIC'}, {
                                value: 'D', label: 'DATE'
            ...
            return (<span
                className={oldFormData?.fields?.some(filedValue => filedValue.key === record.key && filedValue.nameDbf === text && filedValue.formatDbf === record.formatDbf) ? '' : 'fst-italic text-decoration-underline'}>
                        <span className='d-flex justify-content-between w-100'>
                            {text ? (<span>{text}</span>) : (<span className="text-secondary">(Пусто)</span>)}
                            <small className='fw-bold'>{transformValue(record.formatDbf || 'C')}</small>
                            </span>
                        </span>)
        },
    }, {
        ...
        render: (text, record) => {
            if (record.key === selectedRowKey) {
                return <InputNumber
                    value={text || 1}
                    disabled={formData && formData.fields && !formData.formats?.some(format => ['xlsx', 'dbf'].includes(format))}
                    required={true}
                    min={1}
                    max={255}
                    changeOnWheel={true}
            ...
            return (<span
                className={oldFormData?.fields?.some(filedValue => filedValue.key === record.key && filedValue.charNumber === text) ? '' : 'fst-italic text-decoration-underline'}>
                        {text}
                    </span>)
        },
    }, {
        ...
        title: <Tooltip placement="topRight" title="Помощь">
            <button className="btn color-purple" type='button' onClick={() => setTourOpen(true)}>
                <FontAwesomeIcon icon={faQuestionCircle} size="lg"/>
            </button>
        </Tooltip>, className: 'text-center', render: (text, record) => <Popconfirm placement="topLeft"
                                                                                    disabled={formData && formData.fields && formData.fields.length <= 1}
                                                                                    title="Вы уверены что хотите удалить это поле?"
                                                                                    onConfirm={() => handleDelete(record.key)}>
            <Tooltip placement="topRight"
                     title={formData && formData.fields && formData.fields.length <= 1 ? 'Невозможно удалить. В таблице должна быть минимум 1 поле' : 'Удалить поле'}>
    ...
    const getFieldsOptionsList = () => {
        ...
        try {
            ...
            options.push({
                value: 'new_row', label: 'Пустое поле'
            })
        } catch (error) {
            console.log(error)
        }
    ...
    const findFieldValue = (fields, key) => {
        ...
        try {
            const field = fields.find(item => item && item.hasOwnProperty(key));
    ...
    return (<>
        ...
        <div className=" d-flex w-100">
                ...
                <div className="d-flex flex-column">
                    ...
                    <div ref={refStatus} className='d-flex align-items-center justify-content-between'>
                        ...
                        <UniversalSelect
                            ...
                            isSearchable={false}
                            options={[{value: false, label: 'Активен'}, {value: true, label: 'Отключён'},]}
                        />
                    </div>
                    <div ref={refSendType} className='d-flex align-items-center justify-content-between'>
                        ...
                        <UniversalSelect
                            ...
                            isSearchable={false}
                            options={[{value: 1, label: 'По услугам'}, {value: 2, label: 'По серверу'},]}
                        />
                    </div>
                    ...
                    {formData.createdAt || formData.updatedAt ? (<>
                        ...
                        <div className='d-flex align-items-center justify-content-between'>
                            <h6>Создано:</h6>
                            <h6>{formData.createdAt || <span
                                className="text-secondary">(Дата отсутствует)</span>} - {formData.create_author ||
                                <span className="text-secondary">(Имя отсутствует)</span>}
                            </h6>
                        </div>
                        <div className='d-flex align-items-center justify-content-between'>
                            <h6>Обновлено:</h6>
                            <h6>{formData.updatedAt || <span
                                className="text-secondary">(Дата отсутствует)</span>} - {formData.update_author ||
                                <span className="text-secondary">(Имя отсутствует)</span>}
                            </h6>
                        </div>
                    </>) : null}
            ...
            <div ref={refServerAndServices} className='d-flex flex-column w-50 ms-2'>
                ...
                />
                {selectedServer ? (<>
                    {formData.send_type === 1 ? (<ServiceByServerSelect
        ...
        <div ref={refTable}>
            <SmartTable
                ...
                data={formData?.fields?.length > 0 ? formData.fields : defaultFields} // ИСПРАВЛЕНО: безопасный доступ ?.
        ...
        <div className="d-flex w-100 justify-content-between align-items-center ">
            <div className=' mt-3 d-flex align-items-center'>
                <Tooltip placement="leftTop" title='Включить отображение поля'>
                    <Switch
                        checked={!!findFieldValue(formData.additional_fields, 'enableTotalpayField')} // ИСПРАВЛЕНО: на контролируемый checked
                ...
                <Tooltip placement="bottom" title='Название поля с итоговой суммой'>
                    <div>
                        <FormInput
                            ...
                            value={findFieldValue(formData.additional_fields, 'totalpayFieldName') || 'ИТОГО:'} // ИСПРАВЛЕНО: контролируемый value
            ...
            <div ref={refAddNewRow} className='w-25 mt-3 d-flex align-items-center'>
                <Select
                    ...
                    size='large'
                    className="w-100"
                    value={addFieldVariant} // ИСПРАВЛЕНО: контролируемый компонент
